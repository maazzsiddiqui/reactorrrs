...
public class WorkStateStore {
	private final Logger log = LoggerFactory.getLogger(WorkStateStore.class);

	private Provider<MongoWrapper> jongoProvider;
	private ObjectMapper mapper = JsonProvider.INSTANCE.getMapper();

	@Inject
	public WorkStateStore(Provider<MongoWrapper> jongoProvider) {
		this.jongoProvider = jongoProvider;
	}

	public String storeState(Work work) {
	...
	public String storeState(String tenant, JsonNode state) {
		...
		jongoProvider.get().getJongo(tenant).getCollection(Constants.JOBS).insert(document);
		return document.get(Constants._ID).asText();
	}
